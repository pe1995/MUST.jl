...
# ╔═╡ aabc7d49-aa82-4ba2-abc4-00a6acc294d7
...
begin
	add_selection = false
	folder = MUST.@in_dispatch "data/DIS_MARCS_E_t45g20m00_v0.1"
end
...
snapshots = available_snapshots[1:1]
...
# ╔═╡ ca7e7d97-3a2b-41ec-bc56-cd64b87647b7
...
function convert_snapshots(snapshots, eos=eos_sq, opa=opa_sq; 
    ...
    for i_s in eachindex(snapshots)
        ...
        if true
			...
			T  = lookup(eos, :lnT, b_s[:d], b_s[:ee])
			kr = lookup(eos, opa, :κ_ross, b_s[:d], b_s[:ee])
...
# ╔═╡ 7129d8e7-eb4e-4b26-9215-34f03d238573
