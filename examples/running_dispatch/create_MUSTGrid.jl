...
central_seeds = Dict{Symbol, Dict{String, Float32}}(   
                        :stellar_params => Dict{String, Float32}("tt_k" => 11000.0, "d_cgs" => log(1.6397e-7), "ee_min" => 5.5), 
                        :newton_params  => Dict{String, Float32}("ee0"=>5.3)
                    )

relative_lims = Dict{Symbol, Dict{String, Float32}}(   
                        :stellar_params => Dict{String, Float32}("tt_k" => 0.2, "d_cgs" => 0.2, "ee_min" => 0.001), 
                        :newton_params  => Dict{String, Float32}("ee0"=>0.001)
                    )
...
MUST.create_namelists!(phase1_grid, phase="phase1")

# Save the intermediate results
CSV.write("summary.csv", phase1_grid.info)

# Run the first phase and check for completion
run!(phase1_grid, threads=threads, memMB=mem)
...
MUST.create_namelists!(phase2_grid, phase="phase2")

# Save the intermediate results
CSV.write("summary.csv", phase2_grid.info)

# Run the first phase and check for completion
run!(phase2_grid, threads=threads, memMB=mem)
