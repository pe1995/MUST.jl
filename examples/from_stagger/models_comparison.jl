...
begin
	...
	for (i, snap) in enumerate(snapshots_τ)
		mtrc[i] = metric(snap)
	end

	hline!(
		[metric(stagger_τ) ./ first(mtrc)], ls=:dash, color=:grey, 
		label="Stagger"
	)
	
	plot!(
		x_mtrc, mtrc ./first(mtrc), color=:black, marker=:square, 
		markercolor="white", markerstrokewidth=4,
		markersize=8, lw=2,
		label="DISPATCH"
	)

	plot!(ylim=(0.96, 1.03), xlim=(4,20))
	plot!(ylabel="<T>(τ = 1) / <T>(τ = 1, low res.)")
