...
function absdat_abundances(;α=0.0, default="./input_multi3d/abund_magg", eles...)
	...
	new_name = default
	if α != 0.0
		for ele in α_elements
			iele = findfirst(ele .== abund_default[:, 1])
			if !isnothing(iele)
				abund_default[iele, 2] += α
			else
	...
	if new_name != default
		open(new_name, "w") do f
			for i in axes(abund_default, 1)
				line = MUST.@sprintf "%<4s %<.3f\n" abund_default[i, 1] abund_default[i, 2]
				write(f, line)
			end
		end
	else
...
begin
	...
	# modify chemical composition by element name (beyond [Fe/H] with absdat file, give as [X/Fe])
	FeH = -4.0
	absdat_file = absdat_abundances(
		α=0.4, 
		C=3.0,
