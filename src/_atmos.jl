# ============================================================================= 
# MUST.jl Atmosphere Data Structures
# ============================================================================= 
...
# ============================================================================= 
# Functionality for AtmosphericParameters
# ============================================================================= 
...
# ============================================================================= 
# Constructing Box from Space and vice versa
# ============================================================================= 
...
# ============================================================================= 
# Constructing Box from python snapshots (legacy)
# ============================================================================= 
...
# ============================================================================= 
# Compute plane statistics of a Box object
# ============================================================================= 
...
# ============================================================================= 
# Store/Manipulate models in HDF5
# ============================================================================= 
...
# =============================================================================
# Box loading functionality based on naming conventions
# =============================================================================
...
"""
pick_snapshot(folder::String, i; box_name="box", tau_name="tau", kwargs...) = begin
    cs = converted_snapshots(folder; box_name=box_name, tau_name=tau_name)
    pick_snapshot(cs, i; kwargs...)
end
...
# ============================================================================= 
# Convert from geometrical scale to optical depth 
# ============================================================================= 

"""
    height_scale(b::MUST.Box, new_scale; logspace=true, dont_log=[:T, :ux, :uy, :uz, :ee, :e], kwargs...)

Switch the height scale of a box. Creates a new box by reducing the old box by column for every new height value.
"""
function height_scale(b::MUST.Box, new_scale; logspace=true, dont_log=[:T, :ux, :uy, :uz, :ee, :e], kwargs...)
    N_points = size(b.z, 3)
    TT = eltype(b.z)
    new_box = deepcopy(b)
    ...
    lgh = log.(10, h_scale)
    lgn = log.(10, b[new_scale])
    sorting, interps = interpolators(lgn, lgh; kwargs...)

    # Save loop allocations
    sortingz = zeros(Int, size(b.x, 3))
    parameters = keys(new_box.data) |> collect
    ...
    @inbounds for j in axes(b.x, 2)
        @inbounds for k in axes(b.x, 1)
            ...
            for q in parameters
                ...
                @inbounds new_box.data[q][k, j, :] .= if all(x -> x > 0.0, view(b.data[q], k, j, :)) && !(q in dont_log)
                    exp.(
                        gevaluate!(
                            interps[k, j], 
                            log.(view(b.data[q], k, j, sortingz)),
                            return_copy=false
                        )
                    )
                else
...
# for legacy code
height_scale_fast = height_scale
...
# =============================================================================
# Functions for snapshot statistics
# =============================================================================
...
function average!(b::Box; depth=true, scale=:z)
    ...
    flip_average!(z, ρ, T, ne, pe, vmic; depth=depth, scale=scale)
end

function flip_average!(z, ρ, args...; depth=false, scale=:z)
    d = ρ   
...
# =============================================================================
# Inverting snapshots height scales
# =============================================================================
...
include("_box_spectra.jl")

# =============================================================================
# Convert from Dispatch entirely in Julia 
# =============================================================================
